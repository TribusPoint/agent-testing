/* ═══════════════════════════════════════════
   Agent Testing · UI4 — development variant
...
/** Unified questions: structured rows with a tester id (generated or manual). */
...
function showHome() {
  ...
  headerTitle.textContent = "Agent Testing \u00b7 UI4";
  currentTest = null;
  loadTests();
}
...
function handleWsMessage(msg) {
  ...
  if (msg.type === "questions_done") {
    if (currentTest && msg.testId === currentTest.id) {
      ...
      pqStatus.textContent = "";
      sqStatus.textContent = `${n} question${n !== 1 ? "s" : ""} ready (testers × dimensions × profiles).`;
...
function renderQClassic() {
  ...
  questionsDashboard.innerHTML = `
    ...
    <div id="uq-add-slot"></div>
    ${qs.length ? `<div class="table-wrap"><table class="data-table"><thead><tr><th>#</th><th>Question</th><th>Tester</th><th>Dimension</th><th>Value</th><th>Profile</th><th>Expected</th><th></th></tr></thead><tbody>${rows}</tbody></table></div>` : '<p class="muted">No questions yet. Generate testers, dimensions, profiles, then generate questions.</p>'}`;
...
function updateMatrixButton() {
  ...
  if (allQ.length > 0) {
    ...
    matrixInfo.textContent = "No questions yet. Use the Questions tab after testers, dimensions, and profiles exist.";
