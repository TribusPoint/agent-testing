/* ═══════════════════════════════════════════
   Agent Testing — site-based persona testing
   with evaluation, dimensions, reports
   ═══════════════════════════════════════════ */
...
function resetInputMemoryForNewTest() { inputMemory.personaCount = INPUT_DEFAULTS.personaCount; inputMemory.questionCount = INPUT_DEFAULTS.questionCount; }
...
/** Unified questions: structured rows with a persona id (generated or manual). */
function getTestQuestions() { return (currentTest?.questions || []).filter((q) => q.type === "structured" && q.personaId); }
...
function showHome() {
  ...
  headerTitle.textContent = "Agent Testing";
  currentTest = null;
  loadTests();
}
...
function showTest(test) {
  ...
  headerTitle.textContent = test.name || test.url;
...
function switchTab(name) {
  ...
  if (name === "personasDims") {
    const hasAnalysis = !!currentTest?.analysis;
    personaGuard.classList.toggle("hidden", hasAnalysis);
    renderPersonasDimsView();
  }
  if (name === "runs") { renderRunHistory(); updateMatrixButton(); }
...
function handleWsMessage(msg) {
  ...
  if (msg.type === "report") {
    renderLiveReportCards(msg.report);
  }
  ...
  if (msg.type === "analysis_done") {
    if (currentTest && msg.testId === currentTest.id) {
      ...
      headerTitle.textContent = currentTest.name;
      renderAnalysisDashboard();
  ...
  if (msg.type === "personas_done") {
    if (currentTest && msg.testId === currentTest.id) {
      ...
      renderPersonasDimsView();
      renderQuestionsView();
      setPersonaStatus(`${msg.personas.length} personas generated.`, "ok");
      updateMatrixButton();
    }
  ...
  if (msg.type === "dimensions_done") {
    if (currentTest && msg.testId === currentTest.id) {
      ...
      renderPersonasDimsView();
      dimsStatus.textContent = `${msg.dimensions.length} dimensions generated.`;
      dimsStatus.className = "status ok";
    }
  ...
  if (msg.type === "profiles_done") {
    if (currentTest && msg.testId === currentTest.id) {
      ...
      renderPersonasDimsView();
      dimsStatus.textContent = `${msg.profiles.length} profiles generated.`;
      dimsStatus.className = "status ok";
    }
  ...
  if (msg.type === "structured_questions_cleared") {
    if (currentTest && msg.testId === currentTest.id) {
      ...
      sqStatus.textContent = "Questions cleared — regenerating to sync with new dimensions/profiles...";
  ...
  if (msg.type === "questions_done") {
    if (currentTest && msg.testId === currentTest.id) {
      ...
      pqStatus.textContent = "";
      sqStatus.textContent = `${n} question${n !== 1 ? "s" : ""} ready (personas × dimensions × profiles).`;
...
async function loadTests() {
  ...
  testsList.innerHTML = "";
  if (!tests.length) {
    testsList.innerHTML = '<li class="empty">No tests yet. Create one to get started.</li>';
    return;
  }
  for (const t of tests) {
    ...
    li.innerHTML = `
      <div class="test-info">
        ...
        <div class="test-meta">${esc(t.url)} &middot; ${personaCount} personas &middot; ${runCount} runs &middot; ${fmtDate(t.createdAt)}</div>
      </div>
...
function renderAnalysisDashboard() {
  ...
  if (globalClassicView) {
    renderAnalysisClassic(a);
  } else {
    renderAnalysisModern(a);
  }
  wireAnalyzeButton();
  wireViewToggle();
}
...
linkToAnalysis.addEventListener("click", (e) => { e.preventDefault(); switchTab("analysis"); });

// ═════════════════════════════════
//  PERSONAS & DIMENSIONS (Dashboard/Classic)
// ═════════════════════════════════
...
function renderPDModern() {
  ...
  const qAll = getTestQuestions();

  const personaCards = personas.map((p) => {
    const pqc = qAll.filter((q) => q.personaId === p.id).length;
    ...
    return `<div class="pd-persona-card" data-id="${p.id}" style="border-top:3px solid ${color}">
      ...
      <div class="pd-pc-row"><span class="pd-pc-label">Role</span><span class="pd-pc-val">${esc(p.role)}</span></div>
      <div class="pd-pc-row"><span class="pd-pc-label">Goal</span><span class="pd-pc-val">${esc(p.goal)}</span></div>
      <div class="pd-pc-row"><span class="pd-pc-label">Tone</span><span class="pd-pc-val">${esc(p.tone)}</span></div>
  ...
  pdDashboard.innerHTML = `
    ...
    <div class="pd-stats">
      <div class="ad-stat"><div class="ad-stat-value c1">${personas.length}</div><div class="ad-stat-label">Personas</div></div>
      <div class="ad-stat"><div class="ad-stat-value c2">${qAll.length}</div><div class="ad-stat-label">Questions</div></div>
    ...
    <div class="ad-section-title"><span class="ad-section-icon">&#x1F464;</span> Personas</div>
    <div class="pd-controls row">
      <button id="btn-add-persona" class="secondary" type="button">+ Add Persona</button>
      ...
      <button id="btn-generate" class="primary" type="button">Generate Personas</button>
      <button id="btn-clear-personas" class="secondary" type="button">Clear All</button>
    </div>
    <div id="pd-add-persona-slot"></div>
    ${personas.length ? `<div class="pd-persona-grid">${personaCards}</div>` : '<div class="q-empty">No personas yet. Generate or add one above.</div>'}
...
function renderPDClassic() {
  ...
  const personaRows = personas.map((p) => {
    ...
    return `<tr data-id="${p.id}"><td><strong>${esc(p.name)}</strong></td><td>${esc(p.role)}</td><td>${esc(p.goal)}</td><td>${esc(p.tone)}</td><td>${esc(p.knowledgeLevel)}</td><td>${pqc}</td><td><button class="pd-edit-btn" data-pid="${p.id}">Edit</button> <button class="pd-del-btn" data-pid="${p.id}">Del</button></td></tr>`;
  }).join("");
  ...
  pdDashboard.innerHTML = `
    ...
    <div id="pd-add-persona-slot"></div>
    ${personas.length ? `<div class="table-wrap"><table class="data-table"><thead><tr><th>Name</th><th>Role</th><th>Goal</th><th>Tone</th><th>Knowledge</th><th>Qs</th><th></th></tr></thead><tbody>${personaRows}</tbody></table></div>` : '<p class="muted">No personas yet.</p>'}
    <div class="pd-divider"></div>
    <h3>Dimensions</h3>
    <div class="pd-controls row">
      <button id="btn-gen-dims" class="primary" type="button">Generate Dimensions</button>
...
function wirePDEvents() {
  ...
  if (genBtn) genBtn.addEventListener("click", () => {
    ...
    setPersonaStatus("Generating diverse personas...", "running");
  ...
  if (addBtn) addBtn.addEventListener("click", () => {
    ...
    slot.innerHTML = `<div class="card form-card" style="margin:.5rem 0">
      <div class="row" style="flex-wrap:wrap;gap:.4rem">
        ...
        <input id="ap-role" type="text" placeholder="Role" style="flex:1;min-width:120px" />
      ...
      <div class="row" style="margin-top:.4rem;flex-wrap:wrap;gap:.4rem">
        <input id="ap-tone" type="text" placeholder="Tone" style="flex:1;min-width:120px" />
    ...
    slot.querySelector("#btn-save-persona").addEventListener("click", async () => {
      ...
      const body = { name, role: document.getElementById("ap-role").value.trim() || "Visitor", goal: document.getElementById("ap-goal").value.trim() || "General inquiry", tone: document.getElementById("ap-tone").value.trim() || "Neutral", knowledgeLevel: document.getElementById("ap-knowledge").value.trim() || "Beginner" };
      ...
      if (r.ok) { const { persona } = await r.json(); currentTest.personas.push(persona); renderPersonasDimsView(); updateMatrixButton(); setPersonaStatus("Persona added.", "ok"); }
    });
  });

  const clearBtn = pdDashboard.querySelector("#btn-clear-personas");
  if (clearBtn) clearBtn.addEventListener("click", async () => {
    if (!currentTest || !confirm("Clear all personas and their questions?")) return;
    await fetch(`/api/tests/${currentTest.id}/personas`, { method: "DELETE" });
    currentTest.personas = [];
    currentTest.questions = [];
    renderPersonasDimsView(); renderQuestionsView(); updateMatrixButton();
  });
  ...
  pdDashboard.querySelectorAll(".pd-pq-badge").forEach((badge) => {
    badge.addEventListener("click", (e) => { e.stopPropagation(); switchTab("questions"); });
  });
...
function openPersonaEdit(card, persona) {
  ...
  form.innerHTML = `
    ...
    <input name="role" value="${esc(persona.role)}" placeholder="Role" />
    <input name="goal" value="${esc(persona.goal)}" placeholder="Goal" />
    <input name="tone" value="${esc(persona.tone)}" placeholder="Tone" />
  ...
  form.querySelector(".save-btn").addEventListener("click", async () => {
    const body = {};
    for (const field of ["name", "role", "goal", "tone", "knowledgeLevel"]) body[field] = form.querySelector(`[name="${field}"]`).value.trim();
    setPersonaStatus("Saving persona...", "running");
    ...
    if (r.ok) {
      ...
      if (t.ok) {
        const { test } = await t.json();
        currentTest.questions = test.questions;
      }
      setPersonaStatus("Persona updated.", "ok");
      renderPersonasDimsView(); renderQuestionsView(); updateMatrixButton();
    }
  });
...
function renderQModern() {
  ...
  questionsDashboard.innerHTML = `
    ...
    <p class="muted" style="font-size:.8rem;margin-bottom:.6rem">Each question is tied to a persona, dimension value, and profile. Generate after you have personas, dimensions, and profiles.</p>
    <div class="pd-stats">
    ...
    <div class="pd-controls row" style="flex-wrap:wrap">
      <label class="inline-label">Total count <input id="question-count" type="number" min="5" max="100" value="${inputMemory.questionCount}" class="small-input" /></label>
      ...
      <label class="inline-label">Persona <select id="uq-filter-persona"><option value="">All</option>${[...new Set(qs.map((q) => q.persona).filter(Boolean))].map((p) => `<option value="${esc(p)}">${esc(p)}</option>`).join("")}</select></label>
...
function renderQClassic() {
  ...
  questionsDashboard.innerHTML = `
    ...
    <div id="uq-add-slot"></div>
    ${qs.length ? `<div class="table-wrap"><table class="data-table"><thead><tr><th>#</th><th>Question</th><th>Persona</th><th>Dimension</th><th>Value</th><th>Profile</th><th>Expected</th><th></th></tr></thead><tbody>${rows}</tbody></table></div>` : '<p class="muted">No questions yet. Generate personas, dimensions, profiles, then generate questions.</p>'}`;
...
function renderUnifiedQuestionGroups(container) {
  ...
  if (!filtered.length) {
    container.innerHTML = `<div class="q-empty">${qs.length ? "No questions matching filters." : "No questions yet. Generate personas, dimensions, profiles, then generate questions."}</div>`;
    return;
  }
...
function wireQEvents() {
  ...
  if (genQ) genQ.addEventListener("click", () => {
    ...
    if (!currentTest.personas.length) { pqStatus.textContent = "Generate personas first."; pqStatus.className = "status err"; return; }
    ...
    const count = Math.min(100, Math.max(5, Number(inp?.value) || inputMemory.questionCount));
    inputMemory.questionCount = count;
    if (inp) inp.value = String(count);
    genQ.disabled = true;
    pqStatus.textContent = "";
    sqStatus.textContent = "Generating questions across personas, dimensions, and profiles...";
    sqStatus.className = "status running";
  ...
  if (addBtn) addBtn.addEventListener("click", () => {
    ...
    slot.innerHTML = `<div class="card form-card" style="margin:.5rem 0"><div class="row" style="flex-wrap:wrap;gap:.4rem">
      <label class="inline-label">Persona <select id="asq-persona-id"><option value="">Select...</option>${personas.map((p) => `<option value="${p.id}" data-name="${esc(p.name)}">${esc(p.name)}</option>`).join("")}</select></label>
...
function updateMatrixButton() {
  ...
  if (allQ.length > 0) {
    ...
    matrixInfo.textContent = "No questions yet. Use the Questions tab after personas, dimensions, and profiles exist.";
...
function populateMatrixFilters() {
  ...
  matrixFilterPersona.innerHTML = '<option value="">All personas</option>' + testerIds.map((id) => {
...
function showRunDetail(run) {
  ...
  for (const e of run.events || []) {
    addActivityEntry(e.ts, e.message, false);
  }
...
function addRunLog(ts, message) {
  addActivityEntry(ts, message, true);
}
